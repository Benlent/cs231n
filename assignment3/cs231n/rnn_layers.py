...
def rnn_step_backward(dnext_h, cache):
    ...
    #dz = dnext_h * (4 * np.exp(2 * z)) / np.square(1 + np.exp(2 * z))  # N, H
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        h[:, t, :], _ = rnn_step_forward(x[:, t, :], h_prev, Wx, Wh, b)
        h_prev = h[:, t, :]
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        ...
        dx[:, t, :], dh_prev_step, dWx_step, dWh_step, db_step = rnn_step_backward(dh[:, t, :] + dh_prev_step, step_cache)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    x, prev_h, prev_c, Wx, Wh, b, i, f, g, o, next_c = cache
    N, H = prev_h.shape
    da = np.zeros((N, 4 * H))
    H = prev_h.shape[-1]
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    N, T, H = dh.shape
    x, prev_h, prev_c, Wx, Wh, b, i, f, g, o, next_c = cache[T-1]
    D = x.shape[-1]
    dx = np.zeros((N, T, D))
